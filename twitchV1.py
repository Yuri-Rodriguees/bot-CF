...
async def main():
    ...
    bot_nick = 'cf_rank'
    canal = 'yuri_de_saogonsalo'

    try:
        twitch = await Twitch(client_id, client_secret)
        if twitch is None:
            raise Exception("Falha ao criar o objeto Twitch")
        ...
        if chat is None:
            raise Exception("Falha ao criar o objeto Chat")
        ...
        async def on_message(msg: ChatMessage):
            if msg.text.startswith("!rank "):
                ...
                await chat.send_message(canal, resultado[:450])
