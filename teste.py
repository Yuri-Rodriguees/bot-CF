...
def consultar_rank_cf(nickname):
    ...
    if response.status_code == 200:
        ...
        if resultados and len(resultados) > 0:
            ...
            print(f"📛 Nick: {jogador.get('ign')}")
            print(f"🏅 Rank: {jogador.get('tier_group_name')} - {jogador.get('rank')}")
            print(f"⭐ Rank Score: {jogador.get('rank_score')}")
            print(f"📊 Vitórias: {jogador.get('win_cnt')}")
            print(f"❌ Derrotas: {jogador.get('lose_cnt')}")
