...
class Bot(commands.Bot):
    ...
    async def event_message(self, message):
        ...
        # Verifica se a mensagem tem o padrão do Restream com YouTube
        match = re.search(r'\[YouTube: .*?\]\s*!rank\s+(.+)', message.content, re.IGNORECASE)
        if match:
            nickname = match.group(1).strip()
            resultado = consultar_rank_cf(nickname)
            await message.channel.send(resultado[:450])
